...
2. 開発モードをワンコマンドで起動します。
   ...
   Docker 構成をフォアグラウンドで起動したい場合は、次も使えます。
...
4. 一度ビルドして release バイナリを直接実行する場合:
...
2. イメージのビルドが済んでいて、バックグラウンド起動だけしたい場合:
...
6. [http://localhost:8788](http://localhost:8788) を開きます。

7. 永続データは `cc-switch-web-data` volume に保存されます。
