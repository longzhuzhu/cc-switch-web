...
- 作者連絡先（Bilibili）: [https://space.bilibili.com/27619688](https://space.bilibili.com/27619688)
...
| Docker 内で Linux パッケージを作成 | `pnpm build:pkg:l` |
...
4. 本番に近い形でローカル実行する場合:
   ...
   ローカル Web サービスモードでも、CC Switch Web 自身のデータ保存先は CC Switch が使うローカル設定ルートです。
...
ホスト環境を汚さずに Linux 向け配布パッケージを作りたい場合は、次を実行してください。

```bash
pnpm build:pkg:l
```

このコマンドは `docker buildx build` を使って、次のファイルを出力します。

```text
release/docker-linux/cc-switch-web-linux-x64.tar.gz
```

展開前の tar.gz ではなく、未圧縮ディレクトリを直接出したい場合は次を使います。
