...
- 作者联系（哔哩哔哩）：[https://space.bilibili.com/27619688](https://space.bilibili.com/27619688)
...
| Docker 内打 Linux 发布包 | `pnpm build:pkg:l` |
...
4. 以接近生产的方式本地运行：
   ...
   在本地 Web 服务模式下，CC Switch Web 自身的数据默认写入 CC Switch 使用的本地配置根目录：
...
### Docker 内导出 Linux 发布包

如果你希望在不干扰宿主机环境的前提下，直接从 Docker 构建一个 Linux 发布包，可以执行：

```bash
pnpm build:pkg:l
```

默认会通过 `docker buildx build` 导出：
